    ...
    <div className="shell py-6 sm:py-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{t("welcomeLede")}</p>
        </div>
      ...
      {hiring ? null : (
        ...
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <Button type="submit" className="shrink-0 px-7">
              {t("searchButton")}
            </Button>
          </div>
        </form>
      ...
      {/* The same index of the board the landing page opens with, counts and
          all — it replaced two rows of outlined pills that linked to the same
          places without saying whether anything was behind them. */}
      <div className="mt-10">
        <JobBrowse locale={locale} counts={counts} districts={districts} embedded />
      </div>
    </div>
  );
}
